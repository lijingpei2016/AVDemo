...
vertex RasterizerData vertexShader(uint vertexID [[vertex_id]],
                                   constant JPVertex *vertexArray [[buffer(JPVertexInputIndexVertices)]]) {
    RasterizerData out;
...
// JPFragmentTextureIndexTextureY、JPFragmentTextureIndexTextureUV：表示纹理索引。
fragment float4 yuvSamplingShader(RasterizerData input [[stage_in]],
                                  texture2d<float> textureY [[texture(JPFragmentTextureIndexTextureY)]],
                                  texture2d<float> textureUV [[texture(JPFragmentTextureIndexTextureUV)]],
                                  constant JPConvertMatrix *convertMatrix [[buffer(JPFragmentInputIndexMatrix)]]) {
...
// [[texture(index)]]：纹理修饰符；可以加索引：[[texture(0)]] 对应纹理 0，[[texture(1)]] 对应纹理 1；
// JPFragmentTextureIndexTextureRGB：表示纹理索引。
fragment float4 rgbSamplingShader(RasterizerData input [[stage_in]],
                                  texture2d<half> colorTexture [[texture(JPFragmentTextureIndexTextureRGB)]]) {
    constexpr sampler textureSampler (mag_filter::linear, min_filter::linear);
