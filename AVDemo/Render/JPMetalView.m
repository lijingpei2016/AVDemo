...
#import "JPShaderType.h"
#import <CoreVideo/CVPixelBufferPool.h>

// 颜色空间转换矩阵，BT.601 Video Range。
static const matrix_float3x3 JPColorMatrix601VideoRange = (matrix_float3x3) {
...
static const matrix_float3x3 JPColorMatrix601FullRange = (matrix_float3x3) {
...
static const matrix_float3x3 JPColorMatrix709VideoRange = (matrix_float3x3) {
    (simd_float3) {1.164,  1.164, 1.164},
...
static const matrix_float3x3 JPColorMatrix709FullRange = (matrix_float3x3) {
    (simd_float3) { 1.0,    1.0,    1.0},
    (simd_float3) {0.0,    -0.187, 1.856},
    (simd_float3) {1.575,    -0.468, 0.0},
};

@interface JPMetalView () <MTKViewDelegate>
...
@implementation JPMetalView
#pragma mark - LifeCycle
- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (self) {
        _currentViewSize = frame.size;
        _fillMode = JPMetalViewContentModeFit;
        _updateFillMode = YES;
...
- (void)setFillMode:(JPMetalViewContentMode)fillMode {
    // 更改视图填充模式。
    _fillMode = fillMode;
    _updateFillMode = YES;
}
...
- (void)_setupYUVMatrix:(BOOL)isFullRange colorSpace:(CFTypeRef)colorSpace{
    ...
    JPConvertMatrix matrix;
    if (colorSpace == kCVImageBufferYCbCrMatrix_ITU_R_601_4) {
        matrix.matrix = isFullRange ? JPColorMatrix601FullRange : JPColorMatrix601VideoRange;
    }else if (colorSpace == kCVImageBufferYCbCrMatrix_ITU_R_709_2) {
        matrix.matrix = isFullRange ? JPColorMatrix709FullRange : JPColorMatrix709VideoRange;
    }
    matrix.fullRange = isFullRange;
    self.yuvMatrix = [self.mtkView.device newBufferWithBytes:&matrix
                                                          length:sizeof(JPConvertMatrix)
...
- (void)_updaterVertices {
    ...
    self.vertices = [self.mtkView.device newBufferWithBytes:quadVertices
                                                ...
                                                options:MTLResourceStorageModeShared];
    // 获取顶点数量。
    self.numVertices = sizeof(quadVertices) / sizeof(JPVertex);
}
    ...
    if (_pixelBuffer) {
        ...
        if (renderPassDescriptor) {
            ...
            [renderEncoder setVertexBuffer:self.vertices
                                    offset:0
                                   atIndex:JPVertexInputIndexVertices];
            ...
            if (isRenderYUV) {
                ...
                if (textureY != nil && textureUV != nil) {
                    [renderEncoder setFragmentTexture:textureY
                                              atIndex:JPFragmentTextureIndexTextureY];
                    [renderEncoder setFragmentTexture:textureUV
                                              atIndex:JPFragmentTextureIndexTextureUV];
                }
...
#pragma clang diagnostic pop
                ...
                [renderEncoder setFragmentBuffer:self.yuvMatrix
                                          offset:0
                                         atIndex:JPFragmentInputIndexMatrix];
            } else {
                // 生成 rgba 纹理。
                ...
                if (textureRGB) {
                    [renderEncoder setFragmentTexture:textureRGB
                                              atIndex:JPFragmentTextureIndexTextureRGB];
                }
            }
